...
static void gdImageBrushApply(gdImagePtr im, int x, int sy);
static void gdImageTileApply(gdImagePtr im, int x, int y);

unsigned long _GdLastSize=0;/* Ilosc bajtow ostatniej alokacji - do obslugi bledow*/

gdImagePtr gdImageCreate(int sx, int sy)
{
	int i;
	gdImagePtr im;
	_GdLastSize=sizeof(gdImage);
	im = (gdImage *) malloc(sizeof(gdImage));
	if(im==NULL) return NULL;

	_GdLastSize+=(sizeof(unsigned char *) * sx);
	im->pixels = (unsigned char **) malloc(sizeof(unsigned char *) * sx);
	if(im->pixels==NULL) return NULL;
	...
	for (i=0; (i<sx); i++) {
		_GdLastSize+=(sy * sizeof(unsigned char));
		im->pixels[i] = (unsigned char *) calloc(sy, sizeof(unsigned char));
		if(im->pixels[i]==NULL) return NULL;
	}
...
/* MODIFICATION FOR 16bit COMPILERS */
#if !defined( __16BIT__ )
#   define ENABLE_LARGE_STATIC_DATA      1
#else
#   if ((size_t)0xffffffffL)==0xffffffffL
#   define ENABLE_LARGE_STATIC_DATA      1
#   else
...
void gdImageGif(gdImagePtr im, FILE *out)
{
	...
	init_statics();
#if ENABLE_LARGE_STATIC_DATA==0
	htab=(count_int*)calloc(HSIZE,sizeof(count_int));
	codetab=(unsigned short*)calloc(HSIZE,sizeof(unsigned short));
	if(htab==NULL || codetab==NULL)
		{
		fprintf(stderr,"Out of memory for compression buffers!\n");
		exit(1);
		}
#endif
...
void gdImageCopyResized(gdImagePtr dst, gdImagePtr src, int dstX, int dstY, int srcX, int srcY, int dstW, int dstH, int srcW, int srcH)
{
	...
	accum = 0;
	for (i=0; (i < srcW); i++) {
		int got;/* Powinien byc typu int */
		accum += (double)dstW/(double)srcW;
		got = (int)floor(accum);/* got jest typu int / a moze nie */
		stx[i] = got;
		accum -= got;
	}
	accum = 0;
...
int __cdecl gdCompareInt(const void *a, const void *b);
