...
protected:
    ...
    int _calculate() //Zwraca 1, jeśli musial przeliczyc
    {
        ...
        double Za = 0; //Ilość bledow dla Tau_b
        double Zb = 0; //Ilość bledow dla Tau_a
        ...
        {//OBLICZANIE HISTOGRAMU 2D KOINCYDENCJI ZDARZEN
            ...
            if(rows <= 1 || columns <= 1) //jeśli któras ze zmiennych ma jedna klase to nie da się ustalic związku
                goto ERROR;
        ...
        if(table[iHi + 5] != NULL)	//"Normalised Entropy"
        {
            if(Entropy < 0)
                Entropy = -Entropy; //Z sumowania wychodzi ujemna bo ln z ulamków
            /*
            double q=1/double(rows*columns); //Ilość mozliwych slupków sprawiedliwie obdzielona
            assert(Pairs>0 && q>0);
            double MaxS=-(double(rows*columns)*(q*log(q)));
            */
            table[iHi + 5]->change_val(
                    Entropy / log(double(rows) * columns)); //Dzielone przez maksymalna mozliwa dla tej liczby slupków
        }
...
public:
    ...
    void all_subseries_required()	//Alokuje i ewentualnie rejestruje w menagerze wszystkie serie
    {
        ...
        Chi2(); //HiKwadrat
        LevelOfFreedom(); //Ilość stopni swobody ukladu
        T2Czupurow(); //T Czupurowa
        V2Cramer(); //V^2 Cramera - miernik zależnoscia dla zredukowanej (min) liczby stopni swobody
        Entropy();
        NormEntropy();
...
//Acces to "childrens"
    ...
    scalar_source<double> *LevelOfFreedom(const char *format = "Loff(%s,%s)")	//Ilość stopni swobody ukladu
    {
        if(iHi == -1)
            iHi = co_statistics_source::number_of_subseries();
...
//Ile elementów,wartość minimalna i maksymalna
    {
        check_version_(); //Uaktualnia tez wersje podźrodła, jeśli trzeba
