...
///     `get` zwraca wartość "miss" podawana w konstruktorze. Alternatywnie
...
//Klasa czytająca z dowolnej prostokątnej tablicy struktór.
...
//------------------------------------------------
{
    ...
    STRUCT_T *arra; //Tablica struktór
    TYP_METODY skladowa; //Wskaźnik do składowej indywiduum
...
//Sięganie do wyniku funkcji - tu bezparametrowej, ale w klasach potomnych niekoniecznie
    {
        double pom;
        if(cache_ok)
            pom = cache[i];
...
//Daje następną z la*lb liczb!!!
    {
        ...
        if(p == NULL) //Koniec danych
            cache_ok = 1; //Wiec już wszystkie w cachu
