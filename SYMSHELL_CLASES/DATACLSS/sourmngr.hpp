...
/// @file
/// @brief KLASA ZARZĄDCY SERI DANYCH.
...
public:
    ...
    virtual int    set_info(size_t        index, //!< pozycja na liście.
                            ...
                            config_point* fig=NULL //!< Obiekt rysujący punkty serii.
                            )=0;
...
private:
    ...
    private:
        ...
        wb_ptr<config_point>	 fig;   //!< Obiekt wizualizujący punkty danych.

        /// @name MANIPULATORY
        /// @{
        void set(data_source_base* d,int not_men) { clear(); dat=d; not_menage=not_men; } ///< setter.
        data_source_base* get(){ return dat;} ///< a read-only reader.
        void clear()
        {
...
public:
    ...
    /// @{
    wb_dynarray<graph::series_info> make_series_info(int start,.../* ostatnia -1*/) override;	///< Z listy parametrów.
    wb_dynarray<graph::series_info> make_series_info(wb_dynarray<int>) override;				///< Z tablicy dynamicznej.
