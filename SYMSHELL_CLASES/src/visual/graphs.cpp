...
void drawable_base::replot(int need_flush)
{
    ...
    if(zostalo_dosc_y && title && (tit_col != default_transparent || tit_bck != default_transparent))
    {
        ...
        unsigned col1=(tit_col != default_transparent?tit_col:getframe());
        unsigned col2=(tit_bck != default_transparent?tit_bck:getbackground());
...
//juz wiadomo,jesli sie nie zmiesci
    ...
    {
        print_width(x1, (y1 + y2) / 2, x2 - x1, t_colors.start, getbackground(), "%@CTo small area for %ux%u graph", A,
                    B);
        return;
    }
...
// Rysuje wlasciwy wykres a pod nim ewentualnie legende
{
    ...
    if(mode == 2)//Konfiguracja min max dla osi Y
    {
        ...
        {//Awaria - zle parametry
            print_width(x1, (y1 + y2) / 2, x2 - x1, t_colors.start, getbackground(), "%@CInvalid min,max range:<%g,%g>",
        ...
        if(mode == 0 && grubosc > 1)
        {
            ...
            for(i = 0; i < M; i++)
                if(series[i].ptr != nullptr)
                {
                    ...
                    printc(x1 + i * grubosc, y2 - 2 * char_height('0'), color(i), getbackground(), "%s",
                           trunc(bufor, grubosc));
                }
        } else //mode>0
        {
        ...
        if(grubosc > 1)
            for(i = 0; i < M; i++)
                if(series[i].ptr != nullptr)
                {
                    ...
                    printc(x1 + i * grubosc, y2 - char_height('0'), color(i), getbackground(), "%s",
                           trunc(bufor, grubosc));
                }
        y1 += char_height('0');
        y2 -= 2 * char_height('0');
        ...
        if(colors != nullptr)
        {
            ...
            } else
            {
                scale_c.set(min, max, c_range.end - c_range.start +
                                      0.999);//Do kolorow potrzebna poprawka 0.999 w skalowaniu bo nie bedzie bialego (max)
            }
...
//Rysowanie punktow
    ...
    {
        if(CurrConfig !=
           nullptr)//Jesli uzytkownik w ogole zyczy sobie rysowac jakies punkty (bo klasa potomna moze nie chciec!)
        {
