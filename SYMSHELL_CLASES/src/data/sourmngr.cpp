...
// Może być malo efektywna przy wiekszej liczbie seri.
...
long version=pom->data_version(); //Taka ma być tez w pozostalych
...
wb_dynarray<symshell2::graph::series_info> todo(siz+1); //Musi być jedna pusta na koncu
for(i=0;i<siz;i++)
	{
    ...
    symshell2::graph::series_info pom(//nie ma zarzadzac
			...
			gcc_pom2,	//Wskaznik rysika.Jeśli NULL to laczy liniami
			0);									//Nie zarzadzaj!
...
// Tworzenie dynamicznie tablice serii
{
    ...
    wb_dynarray<symshell2::graph::series_info> todo(siz+1); //Musi być jedna pusta na końcu.
    ...
    for(index=start,i=0;i<siz;index=va_arg(list,int),i++)
        ...
        symshell2::graph::series_info pom(
                ...
                gcc_pom2,		//Wskaźnik rysika.Jeśli NULL to laczy liniami
                0);			//Nie zarządzaj!
