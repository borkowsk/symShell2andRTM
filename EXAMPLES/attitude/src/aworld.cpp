...
aworld::aworld(size_t Width,		//Szerokość torusa macierzy agentów
    ...
    : world(log_name,50),
      ...
      MyWidth(Width),
      Agents(Width, Width, NULL), //Wzór jest NULL, więc tworzone są konstruktorem, a nie klonowanie
      MaxSila(max_str),	//Maksymalna siła agenta
      ThresholdStr(str_threshold), //Siła dająca odporność na zmiany
      NofCategories(n_of_cate),	//Liczba kategorii w mapach
...
//Rejestruje pochodne serie, tworzy domyślne "lufciki" i wkłada w "Manager"
{
    ...
    //if(!ClassStat) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    //if(!CorrFS) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    //if(!EntropyFSLog) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    //if(!CorrFSLogR) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    //if(!StressFirstLog) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    //if(!ClassEntropyLog) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    //if(!MainClassLog) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    //if(!WhichMainLog) goto ERROR; //Od C++11 już zawsze "false"!
    ...
    unsigned l_width = Manager.get_width();
    unsigned l_height= Manager.get_height();
    assert(l_width > 50 && l_height > 40); //Najmniejsze sensowne okno
    ...
    //WŁAŚCIWE LUFCIKI:
    graph* pom1=new sequence_graph( l_width / 2. - 1, l_height / 4., l_width - 50, l_height / 2. - 1,
    ...
    //Inne mniej potrzebne wstawiane jako sam tytuł:
    graph* pom=new sequence_graph( l_width / 2. - 1, 1, l_width - 50, l_height / 4. - 1,	//domyślne współrzędne
    ...
    pom=new carpet_graph(1, l_height / 2, l_width / 3, l_height - 1,	//domyślne współrzędne
                            Firsts); //I źródło danych
    pom->set_data_colors(0, 255);
    pom->set_title("Map of current attitude");
    Manager.insert(pom);

    pom=new bars_graph(l_width / 3 + 1, l_height / 2, l_width / 3 * 2, l_height - 1,	//domyślne współrzędne
                            ClassStat);
    pom->set_data_colors(0, 255);
    pom->set_title("Histogram of attitude");
    Manager.insert(pom);

    pom=new manhattan_graph(l_width / 3 * 2 + 1, l_height / 2, l_width, l_height - 1,	//domyślne współrzędne
    ...
    //PRZYCISKI
    pom=new carpet_graph(l_width - 49, 5 * (char_height('X') + RAMKA), l_width, 6 * (char_height('X') + RAMKA),	//domyślne współrzędne
    ...
    pom=new carpet_graph(l_width - 49, 6 * (char_height('X') + RAMKA), l_width, 7 * (char_height('X') + RAMKA),	//domyślne współrzędne
    ...
    pom=new manhattan_graph(l_width - 49, 7 * (char_height('X') + RAMKA), l_width, 8 * (char_height('X') + RAMKA), 	//domyślne współrzędne
    ...
    pom1=new sequence_graph(l_width - 49, 9 * (char_height('X') + RAMKA), l_width, 10 * (char_height('X') + RAMKA),
    ...
    pom1=new sequence_graph(l_width - 49, 10 * (char_height('X') + RAMKA), l_width, 11 * (char_height('X') + RAMKA),
                            1, Sources.make_series_info(
                                            iEntropyFS,
                                                -1
                                            ).get_ptr_val(),
    ...
    pom=new sequence_graph(l_width - 49, 11 * (char_height('X') + RAMKA), l_width, 12 * (char_height('X') + RAMKA),
                           1, Sources.make_series_info(
                                            iCorrFSR, //iCorrFS,???
                                                -1
                                            ).get_ptr_val(),
    ...
    drawable_base* l_pom=new steering_wheel(l_width - 49, 0, l_width, 5 * (char_height('X') + RAMKA), tmp);
    assert(l_pom != NULL);
    ...
    ERROR:// Tu "akcja na niepogodę", ale nie zdefiniowaliśmy, bo już się nie zdarza brak pamięci.
        ; //error_message(...)
}
...
//-------------------------------------
{
    ...
    if(first)
      Log.GetStream()
              ...
              << "\nThreshold of Power = " << Log.separator() << ThresholdStr
              << "\nNumber of Cate. = " << Log.separator() << NofCategories
              << "\nNoise %= " << Log.separator() <<Noise*100
              << "\nSelf = " << Log.separator() << UseSelf
              << "\nNeighborhood= " << Log.separator() << NofNeigh << "/(" << (1 + 2 * NeighRadius) << "*" << (1 + 2 * NeighRadius) << ")\n";
...
//---------------------------------------
{
    ...
    if(SyncChange)
    {
        ...
        while(Full)
        {
            ...
            if(CenterAgent.Power <= ThresholdStr)		// Czy nie ma już immunitetu na zmiany
...
{ 
    ...
    //TABLICA POMOCNICZA
    wb_dynarray<int> CountersOfFirsts(NofCategories);                                   assert(CountersOfFirsts.IsOK());
    //Czyszczenie licznika
    memset(CountersOfFirsts.get_ptr_val(), 0, sizeof(int) * NofCategories);
    ...
    while(Neigh)
    {
        ...
        CountersOfFirsts[PeryAgent.First]+=PeryAgent.Power;
    }
    ...
    if(UseSelf)
    {
        CountersOfFirsts[CenterAgent.First]+=CenterAgent.Power;
    }

    //Szukanie maksimów
    int maxF=0,indF=-1;

    int offset=RANDOM(NofCategories);
    assert(0<=offset && offset < NofCategories); //Jak NofCategories==2 to 0 albo 1 itd..

    for(int g=0; g < NofCategories; g++)
    {
        int h= (g+offset) % NofCategories;
        assert(h>=0 && h < NofCategories);
        //Dodawanie szumu
        if(CountersOfFirsts[h] > 0)
            CountersOfFirsts[h]+=long(DRAND() * Noise * (4.5 * MaxSila));

        //Testowanie
        if(CountersOfFirsts[h] > maxF)
        {
            maxF=CountersOfFirsts[h];
            indF=h;
        }
