...
double def_m_of_weight=0.1; //Średnia waga krawedzi
...
public:
    ...
    public:
        Agent():x(0),y(0),r(1),state(0),pstate(0)
            {}
        void _setstate(double state);        //Bezpośrednie nadawanie stanu - bez funkcji mapujacych
    ...
    friend class Swiat; //Musi miec bezpośredni dostep do pol, zeby przypiac zrodla danych
    };
...
private:
    ...
    double m_of_weight; //Średnia waga krawedzi
...
public:
    ...
    Swiat(  unsigned  WhatMode,
        ...
        double im_of_weight=0.5, //Średnia waga krawedzi
    ...
    dlugosc_logow(100000),
        ...
        m_of_weight(im_of_weight), //Średnia waga krawedzi
    ...
    virtual void make_default_visualisation(area_menager_base&  Menager);//Tworzy domyslne "lufciki" i umieszcza w     
...
void Swiat::simulate_one_step()
{
    ...
    if(mode==1 || mode==2)//Fully connected or from file undirected
    {
        for(unsigned j=0;j<connections.get_size();j++)
        {
            ...
            connections[j].lastact=max(p1,p2);
        }
    }
...
void Swiat::InitialiseFullyConnected(unsigned HowManyAgents)
{
    ...
    for(unsigned countcon=0,i=1;i<HowManyAgents;i++)
    {
...
void Swiat::InitialiseRandomConnected(unsigned HowManyAgents)
{
    ...
    for(unsigned countcon=0,i=0;i<HowManyAgents;i++)
    {
...
void Swiat::InitialiseFromWiesiekFile(const char* FileName)
{
    ...
    if(HowManyAgents>0 && HowManyAgents<1000)
        if(agents.get_size()==HowManyAgents)
        {//Juz bylo czytane
            _MakeStates(m_init_st,r_init_st); //Ustala agentom stany z rozkladu
            for(int c=0;c<connections.get_size();c++)
...
void Swiat::AllocSources() //Tworzy zrodla danych
{
    ...
    pNodeState=new struct_array_source<Agent,double>(agents.get_size(),agents.get_ptr_val(),&Agent::state,_LO("Stan","State"));  //Aktywnosc wezlow
    pNodePreState=new struct_array_source<Agent,double>(agents.get_size(),agents.get_ptr_val(),&Agent::pstate,_LO("Pop.Stan","PrevSt"));  //Dawna aktywnosc wezlow
    ...
    pConnStart=new struct_array_source<Connection,size_t>(connections.get_size(),connections.get_ptr_val(),&Connection::start_node,_LO("Pocz�tki","Starts"));//Indeksy pocz�tk�w linii laczacych wezly sieci
    pConnEnd=new struct_array_source<Connection,size_t>(connections.get_size(),connections.get_ptr_val(),&Connection::end_node,_LO("Ko�ce","Ends"));//Indeksy koncow linii laczacych wezly sieci
    ...
    pConnAcct=new struct_array_source<Connection,double>(connections.get_size(),connections.get_ptr_val(),&Connection::lastact,_LO("Aktywno��","Acctivity"));//Aktywnosci polaczen
...
void Swiat::make_default_visualisation(area_menager_base& Lufciki)
{
    ...
    assert(pom);
    pom->setframe(128);
    pom->settitle(_LO("HISTORIA STAN�W",HISTofSTATES));
    Lufciki.insert(pom);
    }
    ...
    assert(pom);
    pom->setframe(128);
    pom->settitle(_LO("HISTORIA AKTYWNO�CI",HISTofACCT));
    Lufciki.insert(pom);
    }
...
//Bezpośrednie nadawanie stanu - bez funkcji mapujacych
{
    state=istate;
}
...
void MetaExperyment::tworz_lufciki(area_menager& Lufciki) //Generowanie lufcikow demostracyjnych meta-swiata
{
    ...
    if(points.get_size()>0)
    {
        ...
        Sources.insert(_S=new struct_array_source<Arrow,size_t>(arrows.get_size(),arrows.get_ptr_val(),&Arrow::S,_LO("Pocz�tki","Starts")));//Indeksy pocz�tk�w linii laczacych wezly sieci
        assert(_S->get_missing()==UINT_MAX);
        Sources.insert(_E=new struct_array_source<Arrow,size_t>(arrows.get_size(),arrows.get_ptr_val(),&Arrow::E,_LO("Ko�ce","Ends")));//Indeksy koncow linii laczacych wezly sieci
    ...
    assert(pom!=NULL);
    pom->settitle(_LO("FUNKCJE","FUNCTIONS"));
    pom->setframe(20);
    Lufciki.insert(pom);
...
Swiat MojSwiat( //Model symulacyjny z roznymi parametrami
               ...
               def_m_of_weight,//=0.5, //Średnia waga krawedzi
