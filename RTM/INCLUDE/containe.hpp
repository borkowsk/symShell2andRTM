...
/// Szablon zapewniający właściwą semantykę przypisania na wskaźnik.
/// Zamknięty w typie kontenerowym. Używają funkcji lv klas tablicowych.
template<class TT>
class lvptr
//===================================
{
...
operator TT* () 
	{
		return reftoptr; //rvalue (konwersja do TT*)
	}  

TT* operator = (TT* asi) //Symuluje lvptr
	{ 
	if((flags&0x1) && reftoptr!=NULL)
		delete reftoptr; //oczyszczenie wskaźnika
	reftoptr=asi;		//przypisanie na właściwy wskaźnik
	return reftoptr;	//konwersja do TT*
	}	
};

/// Baza kontenerów — głównie definiuje wyliczenia.
...
typedef container_base cbase; //Istotny skrót, bo trzeba nadużywać w odniesieniach do enum's
...
virtual pix 	     first() const=0;
	// return pix to the first object in the collection, or NULL if empty
...
virtual int    	     remove(pix)=0;
	// deleting a current object, return 1 if success
};
...
key_container(void) {}
public:
// Both "key" method return a key for a current object coded in pix
...
// Both []&() method return lvalue associated with a particular key
//V& operator [] (K)=0; // the most efficient indexing only for already existing items
virtual V& operator () (K)=0; // the most smart indexing. Building(?) item if possible
};
